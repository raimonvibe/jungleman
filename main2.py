...
def draw_window(win, player, platforms, fruits, hearts, score): # Draws everything
    ...
    player.draw(win)
    screen.blit( pg.font.SysFont('comics', 60).render(f"Score: {score}", False, (192,255,238)), (100, 0))
    pg.display.update()
...
fruits = pg.sprite.Group() # Makes a group for all platforms and fruits - this is like a list for sprites
hearts = pg.sprite.Group() # Makes a group for all platforms and fruits - this is like a list for sprites
...
while True: # Main game loop
    if randint(0, 40) == 0:
        sprites.append(Fruit(randint(150, 2400), 30, fruits)) # Makes fruit periodically
    ...
    if player.vel.y > 0:
        hits = pg.sprite.spritecollide(player, platforms, False) # Collision with platform
    ...
    for heart in hearts:
        if heart.vel.y > 0:
            winning = pg.sprite.spritecollide(heart, platforms, False) # Collision with platform
    ...
    winning = pg.sprite.spritecollide(player, hearts, True) # Collision with player
    if winning:
    ...
    hits = pg.sprite.spritecollide(player, fruits, True) # Player collision with fruits
    if hits:
        if BANANA_HIT_SOUND.get_num_channels() == 0:
            BANANA_HIT_SOUND.play(maxtime=500)
    ...
    draw_window(screen, player, platforms, fruits, hearts, score) # Draws sprites
